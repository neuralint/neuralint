...
def build_model(input, scope='lenet', reuse=False, training=True):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        pool1 = tf.layers.max_pooling2d(inputs=reg_conv1, pool_size=[2, 2], strides=2)
        ...
        pool2 = tf.layers.max_pooling2d(inputs=reg_conv2, pool_size=[2, 2], strides=2)
        ...
        logits = tf.layers.dense(inputs=dense_reg, units=10, activation=None)
        return logits
