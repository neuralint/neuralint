...
def build_model(input, scope='lenet', reuse=False, training=True):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        reg_conv1 = tf.layers.dropout(inputs=pool1, rate=0.35, training=training)
        conv1_norm = tf.layers.batch_normalization(reg_conv1, training=training)
        

        conv2 = tf.layers.conv2d(inputs=conv1_norm, filters=64, kernel_size=[5, 5],
                                    padding='same', activation=tf.nn.relu, use_bias = False)
        pool2 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2, 2], strides=2)
        reg_conv2 = tf.layers.dropout(inputs=pool2, rate=0.3, training=training)
        conv2_norm = tf.layers.batch_normalization(reg_conv2, training=training)
        
        pool2_flat = tf.layers.flatten(conv2_norm)
        dense = tf.layers.dense(inputs=pool2_flat, units=512, activation=tf.nn.relu, use_bias = False)
