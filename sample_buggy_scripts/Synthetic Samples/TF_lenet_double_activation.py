...
def build_model(input, scope='lenet', reuse=False, training=True):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        dense_act = tf.nn.sigmoid(dense)
        dropout = tf.layers.dropout(inputs=dense_act, rate=0.25, training=training)
