...
def build_model(input, scope='lenet', reuse=False, training=True):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        dense = tf.layers.dense(inputs=pool2_flat, units=512, activation=tf.nn.relu, kernel_initializer=tf.initializers.constant(0.1))
        dropout = tf.layers.dropout(inputs=dense, rate=0.25, training=training)
        logits = tf.layers.dense(inputs=dropout, units=10, activation=tf.nn.softmax, kernel_initializer=tf.initializers.constant(0.1))
        return logits
