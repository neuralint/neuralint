...
def build_model(input, scope='lenet', reuse=False, training=True):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        dropout1 = tf.layers.dropout(inputs=dense1, rate=0.25, training=training)
        dense2 = tf.layers.dense(inputs=dropout1, units=256, activation=tf.nn.relu)
        dropout2 = tf.layers.dropout(inputs=dense2, rate=0.25, training=training)
        dense3 = tf.layers.dense(inputs=dropout2, units=512, activation=tf.nn.relu)
        dropout3 = tf.layers.dropout(inputs=dense3, rate=0.25, training=training)
        logits = tf.layers.dense(inputs=dropout3, units=10, activation=None)
        return logits
