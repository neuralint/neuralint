...
def is_DNN_layer(layer_type):
    supported_layer_types = ['conv2d','max_pooling2d', 'avg_pooling2d', 
                             'flatten', 'reshape', 'dense', 'dropout', 
                             'batch_normalization']
    supported_layer_types.extend(Activation.activations)
    if layer_type in supported_layer_types:
        return True
    return False
...
class DNN_Program:
    ...
    def write_graph_node(self, filename):
        ...
        with open(f'TF_graphs/{filename}.gst', 'w') as f:
...
class Optimizer:
    optimizers = {'sgd':'GradientDescentOptimizer', 'rmsprop':'RMSPropOptimizer', 
                  'momentum':'MomentumOptimizer', 'adagrad':'AdagradOptimizer',
                  'ftrl':'FtrlOptimizer', 'adam':'AdamOptimizer', 'adadelta':'AdadeltaOptimizer'}
...
class Loss:
    ...
    regression_losses = ['hinge_loss', 'huber_loss', 'mean_squared_error', 'absolute_difference']
    classification_losses = ['log_loss', 'sigmoid_cross_entropy', 'softmax_cross_entropy', 'sparse_softmax_cross_entropy']
...
class DNN:
    ...
    def append_layer(self, layer_name, layer_type, params):
        if layer_type == 'conv2d':
        ...
        elif layer_type in ['max_pooling2d', 'avg_pooling2d']:
            ...
            reshape = Reshape(layer_name, params_dict['tensor'], params_dict['shape'])
        ...
        elif layer_type in ['softmax', 'sigmoid', 'relu', 'elu', 'leaky_relu', 'selu']:
            args_names = Activation.params[layer_type] if layer_type in Activation.params else Activation.params['default']
            params_dict = fill_in_params(params, args_names=args_names)
            inputs = params_dict[args_names[0]]
            activation = Activation(layer_name, inputs, layer_type)
            self.layers.append(activation)  

    def prepend_layer(self, layer_name, layer_type, params):
        if layer_type == 'conv2d':
            params_dict = fill_in_params(params, args_names=Convolutional.params)
            conv = Convolutional(layer_name, params_dict['inputs'], params_dict) 
            self.layers.insert(0, conv)  
        elif layer_type == 'max_pooling2d' or layer_type == 'avg_pooling2d':
            params_dict = fill_in_params(params, args_names=Pooling.params)
            pool = Pooling(layer_name, params_dict['inputs'], params_dict)
            ...
            params_dict = fill_in_params(params, args_names=Flatten.params)
            flat = Flatten(layer_name, params_dict['inputs'])
            self.layers.insert(0,flat)  
        elif layer_type == 'reshape':
            params_dict = fill_in_params(params, args_names=Reshape.params)
            reshape = Reshape(layer_name, params_dict['tensor'], params_dict['shape'])
            self.layers.insert(0,reshape)  
        elif layer_type == 'dense':
            params_dict = fill_in_params(params, args_names=Dense.params)
            dense = Dense(layer_name, params_dict['inputs'], params_dict)
            self.layers.insert(0,dense)  
        elif layer_type == 'dropout':
            params_dict = fill_in_params(params, args_names=Dropout.params)
            dropout = Dropout(layer_name, params_dict['inputs'], params_dict['rate'])
            self.layers.insert(0,dropout)  
        elif layer_type == 'batch_normalization':
            params_dict = fill_in_params(params, args_names=Batchnorm.params)
            batchnorm = Batchnorm(layer_name, params_dict['inputs'])
            self.layers.insert(0,batchnorm)  
        elif layer_type in ['softmax', 'sigmoid']:
            args_names = Activation.params[layer_type] if layer_type in Activation.params else Activation.params['default']
            params_dict = fill_in_params(params, args_names=args_names)
            inputs = params_dict[args_names[0]]
            activation = Activation(layer_name, inputs, layer_type)
            self.layers.insert(0,activation)  
    ...
    def get_last_layer_idx(self, layer_name):
        for layer_idx in range(len(self.layers)-1, 0, -1):
            if layer_name in self.layers[layer_idx].name:
                return layer_idx + 1
...
class Convolutional(Learning_Layer):
    params = ['inputs', 'filters', 'kernel_size', 'strides', 'padding']
    ...
    def __init__(self, name, input_layer_name, params):
        ...
        self.kernel_size = params['kernel_size'] if 'kernel_size' in params else None
    ...
    def gen_graph_nodes_and_edges(self, node_id, node_counter):
        ...
        graph_edges += "\t\t<edge from=\"{}\" to=\"{}\">\n\t\t\t<attr name=\"label\">\n\t\t\t\t<string>let:type = string:\"{}\"</string>\n\t\t\t</attr>\n\t\t</edge>\n".format(
                next_node_id, next_node_id, 'conv2d')
...
class Dense(Learning_Layer):
    params = ['inputs', 'units', 'activation', 'use_bias', 'kernel_initializer', 'bias_initializer', 'kernel_regularizer', 'bias_regularizer']
    def __init__(self, name, input_layer_name, params):
...
class Pooling(Processing_Layer):
    params = ['inputs', 'pool_size', 'strides', 'padding']
    def __init__(self, name, input_layer_name, params):
        super(Pooling, self).__init__(name, input_layer_name)
        self.pool_size = params['pool_size'] if 'pool_size' in params else [2, 2]
        self.set_strides(params)
        self.padding = params['padding'] if 'padding' in params else 'valid'
    ...
    def gen_graph_nodes_and_edges(self, node_id, node_counter):
        ...
        graph_edges += "\t\t<edge from=\"{}\" to=\"{}\">\n\t\t\t<attr name=\"label\">\n\t\t\t\t<string>type:Parameters</string>\n\t\t\t</attr>\n\t\t</edge>\n".format(
                next_node_id, next_node_id)
        graph_edges += "\t\t<edge from=\"{}\" to=\"{}\">\n\t\t\t<attr name=\"label\">\n\t\t\t\t<string>let:type = string:\"{}\"</string>\n\t\t\t</attr>\n\t\t</edge>\n".format(
                next_node_id, next_node_id, 'max_pooling2d')
...
class Flatten(Processing_Layer):
    ...
    def infer_output_size(self):
        batch_size, in_height, in_width, in_depth = self.input_size
        return [batch_size, int(in_height * in_width * in_depth)]
...
class Activation(Processing_Layer):
    activations = ['elu', 'relu', 'selu', 'relu6', 'leaky_relu', 'tanh', 'sigmoid', 'softmax']
    params = {'default':['features'], 'softmax':['logits','axis'], 'sigmoid':['x']}

    def __init__(self, name, input_layer_name, activation, axis=None):
